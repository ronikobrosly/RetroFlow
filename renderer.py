...
class FlowchartRenderer:
    ...
    def __init__(self, box_width=11, box_height=3, 
                 horizontal_spacing=4, vertical_spacing=3):
    ...
    def render(self, graph, layout) -> str:
        ...
        canvas_width = (layout_width * (total_box_width + self.horizontal_spacing) 
                       + self.horizontal_spacing)
        canvas_height = (layout_height * (total_box_height + self.vertical_spacing) 
        ...
        for node, (layer_x, layer_y) in layout.positions.items():
            pixel_x = self.horizontal_spacing + layer_x * (total_box_width + self.horizontal_spacing)
            pixel_y = self.vertical_spacing + layer_y * (total_box_height + self.vertical_spacing)
    ...
    def _draw_connections(self, graph, layout):
        ...
        for source, target, is_feedback in edges:
            ...
            if is_feedback:
                ...
                self._draw_feedback_arrow(src_center_x, src_bottom_y, 
                                         tgt_center_x, tgt_top_y)
            else:
                # Draw normal arrow
                self._draw_arrow(src_center_x, src_bottom_y, 
                               tgt_center_x, tgt_top_y)
